...
def load_session_history(session_id: str) -> tuple[List[Dict], str | None, str | None, str | None, str | None]:
    """
    Loads session history and metadata from JSON file.
    Returns:
        (history, system_role, assistant_name, title, error)
        History format: [{"role": "user|model", "parts": [{"text": "..."}]}, ...]
    """
    
    log_filename = os.path.join(LOG_DIR, f"{session_id}-log.json")
    if not os.path.exists(log_filename):
        return [], None, None, None, f"Session log file '{log_filename}' does not exist. Starting new session."

    try:
        with open(log_filename, 'r', encoding='utf-8') as f:
            log_data = json.load(f)
    except json.JSONDecodeError:
        return [], None, None, None, f"Cannot decode log file '{log_filename}'. Starting new session."
    ...
    assistant_name = log_data.get('assistant_name')
    title = log_data.get('title')
    return history, system_role, assistant_name, title, None

def save_session_history(session_id: str, history: List[Dict], system_prompt: str, assistant_name: str, model_name: str, title: str | None) -> tuple[bool, str | None]:
    """
    ...
    """
    # Always save (also empty/incomplete) to persist assistant metadata
